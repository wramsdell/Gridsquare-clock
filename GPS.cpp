...
void GPSDateTime::decodeType() {
  ...
  if (tmp.equals(GPS_CODE_ZDA) || tmp.equals(GPS_CODE2_ZDA)) {
    ...
    sawGSV = true;
    validCode = inGSV;
  } else if (tmp.length() == 5 && tmp[2] == 'G' && tmp[3] == 'G' && tmp[4] == 'A') {
    validCode = inGGA;
  } else {
    validCode = waitDollar;
  }
...
void GPSDateTime::decodeGGA() {
  int latDegrees = 0, longDegrees = 0;
  switch(count_)
  { //count_ is the N-1st field
    case 2:
      latitude = tmp.toFloat();
      latDegrees = (int)latitude/100;
      latitude = latDegrees+((latitude-(latDegrees*100))/60);
      break;
    case 3:
      if (tmp=="S") latitude = -latitude;
      break;
    case 4:
      longitude = tmp.toFloat();
      longDegrees = (int)longitude/100;
      longitude = longDegrees+((longitude-(longDegrees*100))/60);
      break;
    case 5:
      if (tmp=="W") longitude = -longitude;
      break;
  }
}
